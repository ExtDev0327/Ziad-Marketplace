...
describe("ERC20 New Bid Tests", function () {
  ...
  beforeEach(async function () {
    ...
    erc20 = await ERC20.deploy("Mock ERC20", "CrzErc20");
    ...
    await erc20.connect(user1).approve(auction.address, tokenAmount);
    await erc20.connect(user2).approve(auction.address, tokenAmount);
    await erc20.connect(user3).approve(auction.address, tokenAmount);
  });
  ...
  describe("Test make bids and bid requirements", function () {
    ...
    it("should not allow bidder to withdraw if min price exceeded", async function () {
      await auction
        ...
        .makeBid(erc721.address, tokenId, erc20.address, minPrice);
      await expect(
        auction.connect(user2).withdrawBid(erc721.address, tokenId)
    ...
    it("should allow seller take highestbid and conclude auction", async function () {
      ...
      await auction.connect(user1).takeHighestBid(erc721.address, tokenId);
  ...
  describe("Test underbid functionality", function () {
    ...
    it("should reset bids when bidder withdraws funds", async function () {
      await auction.connect(user2).withdrawBid(erc721.address, tokenId);
      result = await auction.nftContractAuctions(erc721.address, tokenId);
    ...
    it("should not allow other users to withdraw underbid", async function () {
      await expect(
        auction.connect(user3).withdrawBid(erc721.address, tokenId)

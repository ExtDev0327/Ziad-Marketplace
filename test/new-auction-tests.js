...
describe("NFTAuction", function () {
  ...
  it("transferring nft token directly to contract", async function () {
    await erc721
      .connect(user1)
      .transferFrom(user1.address, auction.address, tokenId);
    expect(await erc721.ownerOf(tokenId)).to.equal(auction.address);
  });
  ...
  describe("Test when no bids made on new auction", function () {
    ...
    it("should reset auction if NFT transferred to different owner", async function () {
      ...
      result = await auction.nftContractAuctions(erc721.address, tokenId);
    ...
    it("should reset auction when Auction withdrawn", async function () {
      await auction.connect(user1).withdrawAuction(erc721.address, tokenId);
      let result = await auction.nftContractAuctions(erc721.address, tokenId);
      expect(result.minPrice.toString()).to.be.equal(
    ...
    it("should not allow other users to withdraw Auction", async function () {
      await expect(
        auction.connect(user2).withdrawAuction(erc721.address, tokenId)
    ...
    it("should not allow seller to take highest bid when no bid made", async function () {
      await expect(
        auction.connect(user1).takeHighestBid(erc721.address, tokenId)
    ...
    it("should allow users to query NFT depositer", async function () {
      expect(await auction.ownerOfNFT(erc721.address, tokenId)).to.be.equal(
        user1.address
      );
    });
    it("should revert users querying undeposited NFT ", async function () {
      await expect(auction.ownerOfNFT(erc721.address, 7)).to.be.revertedWith(
        "NFT not deposited"
      );
    });
    it("should not allow user to withdraw 0 failedCredit Balance", async function () {
      await expect(
        auction.connect(user3).withdrawAllFailedCredits()
